...
exports.findOneUser = (req, res, next) => {
  ...
  User.findOne({ where: { id: req.params.id } })
    ...
    .then(() => {
      Post.count({ where: { userId: req.params.id } }).then((total) => {
        userData.totalPosts = total;
      });
    })
    .then(() => {
...
exports.deleteOneUser = (req, res, next) => {
  ...
  if (req.query.isAdmin) {
    ...
    Post.destroy({ where: { UserId: req.query.uid } });
    Comment.destroy({ where: { UserId: req.query.uid } })
      .then((res) => {
        res
          .status(200)
          .json({
            message: "User, its posts and its comments have been destroyed",
          });
      })
